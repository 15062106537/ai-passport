...
- 重构为手机式桌面系统：新增 `main/launcher.c` 桌面启动器（顶部状态栏电量 + 可滚动彩色应用图标网格 + 三键导航），`main.c` 改为 `app_entry_t` 应用注册表与统一返回。语音助手 `demo_voice.c` 改为对接 jarvis-brain 后端（按住 OK 录音 → POST /speak → 收裸 PCM 直接播放）。新增三个三键小游戏：`demo_dice.c`（骰子/双骰/硬币/抽号）、`demo_reaction.c`（反应力测试）、`demo_simon.c`（记忆大师，带喇叭音效）。
