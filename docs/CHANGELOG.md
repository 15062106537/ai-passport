...
- Refactored into a phone-style desktop: added `main/launcher.c` (top status bar with battery, scrollable colored app-icon grid, three-button navigation); `main.c` now uses an `app_entry_t` app registry with unified back navigation. Reworked `demo_voice.c` to talk to the jarvis-brain backend (hold OK to record → POST /speak → stream raw PCM straight to the speaker). Added three button-playable mini-games: `demo_dice.c` (dice / double dice / coin / pick), `demo_reaction.c` (reaction test), and `demo_simon.c` (Simon memory game with speaker tones).
